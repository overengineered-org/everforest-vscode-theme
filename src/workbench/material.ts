...
function semanticStatusColorForWorkbenchIdentifier(
  ...
  if (/(warning|conflict|untracked)/.test(normalizedIdentifier)) return palette.yellow;
  if (/(success|passed|added|inserted|covered)/.test(normalizedIdentifier)) return palette.green;
  if (/(info|modified|remote|link)/.test(normalizedIdentifier)) return palette.blue;
  return undefined;
}
...
function createDocumentedWorkbenchColorFallbacks(
  ...
  const accessibleNeutralForeground = appearance === "dark" ? palette.fg : "#59646c";
  const accessibleMutedForeground = appearance === "dark" ? palette.fg : "#59646c";
  const statusBackgroundForeground = "#1b2024";
  ...
  return Object.fromEntries(
    documentedWorkbenchColorContract.identifiers.map((workbenchColorIdentifier) => {
      const normalizedIdentifier = workbenchColorIdentifier.toLowerCase();
      const semanticStatusColor = semanticStatusColorForWorkbenchIdentifier(
      ...
      if (normalizedIdentifier.includes("background")) {
        if (semanticStatusColor) {
          ...
          return [
            workbenchColorIdentifier,
            hasPairedForeground ? semanticStatusColor : `${semanticStatusColor}30`,
          ];
        }
      ...
      if (/(border|outline|separator|stroke|ruler)/.test(normalizedIdentifier)) {
        return [workbenchColorIdentifier, semanticStatusColor ?? palette.bg4];
      }
      if (semanticStatusColor) {
        ...
        return [
          workbenchColorIdentifier,
          hasPairedBackground ? statusBackgroundForeground : semanticStatusColor,
        ];
      }
      if (/(inactive|disabled|placeholder|deemphasized)/.test(normalizedIdentifier)) {
        return [workbenchColorIdentifier, accessibleMutedForeground];
      }
      return [workbenchColorIdentifier, accessibleNeutralForeground];
    })
  );
}
